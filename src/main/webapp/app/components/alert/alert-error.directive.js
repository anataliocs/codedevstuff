(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('codedevstuffApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-codedevstuffApp-error');
                var entityKey = httpResponse.headers('X-codedevstuffApp-params');
                if (errorHeader) {
